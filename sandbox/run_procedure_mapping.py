...
PROCEDURE_ORIGINAL_TERM_COLUMN = "original_source_name"
...
def main() -> None:
    ...
    vector_search_results_file = project_root / "sandbox" / "procedure_results"  / "vector_search_results_ot.csv"
    if vector_search_results_file.exists():
        vector_search_results = pd.read_csv(vector_search_results_file)
        print("Loaded vector search results from file.")
    else:
        ...
        vector_search_results = concept_searcher.search_terms(unmatched_terms, term_column=PROCEDURE_ORIGINAL_TERM_COLUMN)
        vector_search_results.to_csv(vector_search_results_file, index=False)
