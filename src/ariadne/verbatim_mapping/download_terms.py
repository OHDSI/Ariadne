...
def _create_query(engine: Engine, config: Config) -> Select:
    ...
    if not vocabulary_schema:
        raise Exception("vocabulary_schema environment variable not set")
...
def download_terms() -> None:
    ...
    if "vocab_connection_string" not in os.environ:
        raise Exception("vocab_connection_string environment variable not set")
    ...
    with source_engine.connect() as source_connection:
        ...
        while True:
            ...
            logging.info(
                f"Inserted {len(chunk)} rows, total inserted: {total_inserted}"
            )
