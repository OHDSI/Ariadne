...
from ariadne.utils.config_drug_mapping import DrugMapperConceptClassConfig
...
def _build_test_config(tmp_path):
    ...
    for class_key, label in class_key_to_label.items():
        concept_classes[class_key] = DrugMapperConceptClassConfig(
            terms_folder=Path(tmp_path) / f"terms_{label.replace(' ', '_').lower()}",
            verbatim_mapping_index_file=Path(tmp_path) / f"index_{label.replace(' ', '_').lower()}.pkl",
            domain_ids=["Drug"],
            standard_concept=True,
            vocabularies=None,
            concept_class_ids=[label],
            system_prompt=f"Prompt for {label}",
        )

    return SimpleNamespace(
        system=SimpleNamespace(
            log_folder=Path(tmp_path) / "logs",
            llm_mapper_responses_folder=Path(tmp_path) / "responses",
            download_batch_size=1000,
            max_cores=1,
        ),
        verbatim_mapping=SimpleNamespace(
            substrings_to_remove=[],
            standard_concept_filter=SimpleNamespace(include_synonyms=True),
        ),
        vector_search=SimpleNamespace(max_candidates=25),
...
def test_drug_mapper_runs_class_specific_pipeline(monkeypatch, tmp_path):
    ...
    def fake_download_terms(config):
        download_calls.append(config.system.terms_folder)
